...
const fs = require("fs");
const {getDbClient, logDiff, isDbClient} = require("./utils");
...
class DdlManager {
    static async migrate({db, diff, throwError}) {
        ...
        for (let i = 0, n = createComments.length; i < n; i++) {
            let ddlSql = "";
            let comment = createComments[i];

            ddlSql += CommentOn.comment2sql(comment);
