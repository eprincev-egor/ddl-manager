...
class DbState {
    ...
    async unfreezeAll() {
        ...
        this.functions.forEach(func => {
            let comment = findFunctionComment(dbComments, func);
        ...
        this.triggers.forEach(trigger => {
            let comment = findTriggerComment(dbComments, trigger);
    ...
    static getUnfreezeFunctionSql(func, comment) {
        ...
        let funcIdentifySql = CreateFunction.function2identifySql( func );
        return `
            comment on function ${ funcIdentifySql } is '${prefix}ddl-manager-sync'
        `;
    }
    ...
    static getUnfreezeTriggerSql(trigger, comment) {
        ...
        return `
            comment on trigger ${ triggerIdentifySql } is '${prefix}ddl-manager-sync'
        `;
    }
}
