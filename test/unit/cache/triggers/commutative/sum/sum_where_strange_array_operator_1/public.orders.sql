    ...
    if TG_OP = 'UPDATE' then
        ...
        then
            update companies set
                orders_total = orders_total - coalesce(old.profit, 0)
            where
                cm_get_deleted_elements(old.companies_ids, new.companies_ids) <@ array[companies.id];
        end if;
        ...
        then
            ...
            where
                cm_get_inserted_elements(old.companies_ids, new.companies_ids) <@ array[companies.id];
        end if;
