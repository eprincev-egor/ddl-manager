    ...
    if TG_OP = 'UPDATE' then
        ...
        then
            update companies set
                orders_total = orders_total - coalesce(old.profit, 0)
            where
                array[companies.id] @> cm_get_deleted_elements(old.companies_ids, new.companies_ids);
        end if;
        ...
        then
            ...
            where
                array[companies.id] @> cm_get_inserted_elements(old.companies_ids, new.companies_ids);
        end if;
